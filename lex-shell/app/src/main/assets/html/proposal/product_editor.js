...
var Main = function (_React$Component) {
    ...
    _createClass(Main, [{
        ...
        value: function componentDidMount() {
            ...
            this.state.form = [{}];
            APP.proposal.editProduct(this.state.planId, localStorage.channelId, localStorage.orgId, localStorage.mainProductId, this.state.index, function (r) {
                _this2.state.form[0] = {
                    name: r.name,
            ...
            APP.proposal.viewPlan(this.state.planId, function (plan) {
                APP.proposal.listRiders(_this2.state.planId, localStorage.channelId, localStorage.orgId, localStorage.mainProductId, _this2.state.index, function (r) {
                    r.map(function (v) {
                        var prdForm = {
                            name: v.name,
                            productId: v.code,
                        ...
                        plan.product.map(function (r3, i) {
                            if (r3.productId == v.code && r3.parent == _this2.state.index) {
                                APP.proposal.editProduct(_this2.state.planId, localStorage.channelId, localStorage.orgId, localStorage.mainProductId, i, function (r4) {
        ...
        key: "onValChange",
        value: function onValChange(opt, prdIndex, formIndex, val,w) {
            var _this4 = this;
            var productIdData = ''
            if(w.productId){
                productIdData = w.productId
            }else{
                productIdData = localStorage.editorMainProduct
            }
            var vals = {};
                            ...
                            return React.createElement(
                                ...
                                React.createElement(
                                    ...
                                    v.widget == "number" ? React.createElement("input", { type: "number", placeholder: "请输入" + v.label, value: v.value, onChange: function onChange(e) {
                                            _this5.onValChange(v, i, j, e.target.value,w);
